import br.inatel.impedance_calculator.calculator.Complex;
...
public class MyFrameTest extends JFrame implements ActionListener {
    ...
    public MyFrameTest() {
        ...
        Opanel = new JPanel(); // the panel is not visible in output
        label = new JLabel(Zimage);
        input = new JTextField(30); // accepts up to 30 characters
        input.setFont(input.getFont().deriveFont(Font.PLAIN, 20f));
        calculate = new JButton("="); //Calculate
        copy = new JButton("Ans");
        ...
        if (e.getSource() == copy) {
            try {
                if (!expression.equals("")) {
                    input.setText(EvaluateTest.evaluate(expression).getReal() +
                            "i" +
                            EvaluateTest.evaluate(expression).getImaginary()
                    );
                }
        ...
        if (e.getSource() == calculate) {
            ...
            if (!expression.equals("")) {
                try {
                    ...
                    output.setText(
                            EvaluateTest.evaluate(expression).toString() + "\n" +
                                    ComplexTest.polar(EvaluateTest.evaluate(expression))
                    );

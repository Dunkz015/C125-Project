...
public class EvaluateInput {
    ...
    public static Complex evaluate(String expression) {
        ...
        for (int i = 0; i < input.length; i++)
            if(!condition(input[i]) && input[i] != 'i' && input[i] != 'j' && input[i] != 'J' && input[i] != 'I' &&
                    input[i] != 'L' && input[i] != 'l' && input[i] != 'C' && input[i] != 'c' && input[i] != '+' &&
    ...
    public static Complex applyOp(char op, Complex b, Complex a) {
        return switch (op) {
            case '+' -> Complex.plus(a, b);
            case '|' -> Complex.divides(Complex.times(a, b), Complex.plus(a, b));
